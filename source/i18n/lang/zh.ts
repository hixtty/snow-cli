...
export const zh: TranslationKeys = {
	...
	proxyConfig: {
		...
		hostValidationError: '主机必须是有效的 IP 地址或域名',
		notSet: '未设置',
	...
	codebaseConfig: {
		...
		agentReviewModel: '审查模型:',
		agentReviewModelLlm: 'LLM(基础模型)',
		enabled: '[✓] 已启用',
		disabled: '[ ] 已禁用',
		toggleHint: '(按 Enter 切换)',
		embeddingType: '请求类型:',
	...
	configScreen: {
		...
		scopeSelectSubtitle: '选择要配置的内容：LLM 模型，或独立的决策模型',
		llmConfigEntry: '配置 LLM 模型',
		llmConfigEntrySubtitle: 'API 连接、请求方案、模型、推理参数、令牌与超时',
		scopeSelectHint: '↑↓ 选择 · Enter 进入 · Esc 返回',
		decisionModelConfig: '决策模型配置',
		decisionModelConfigSubtitle:
			'管理独立的决策模型（可配置多个，同一时间仅一个生效）',
	},
	// Decision Models Screen
	decisionModels: {
		title: '决策模型',
		subtitle: '管理独立的决策模型（可配置多个，同一时间仅一个生效）',
		activeModel: '当前激活:',
		none: '无',
		noModelsConfigured: '尚未配置任何决策模型',
		availableModels: '可用决策模型',
		actions: '操作',
		activate: '激活',
		deactivate: '取消激活',
		edit: '编辑',
		...
		addNew: '+ 新增',
		escBack: 'Esc 返回',
		navigationHint: '↑↓ 导航 · ←→ 切换操作 · Enter 确认 · Esc 返回',
		addNewTitle: '新增决策模型',
		editTitle: '编辑决策模型',
		nameLabel: '名称:',
		...
		untitled: '未命名模型',
		notSet: '未设置',
		editingHint: '↑↓ 切换字段 · Enter 编辑 · Ctrl+S 保存 · Esc 返回',
		selectHint: '↑↓ 选择 · Enter 确认',
		confirmDelete: '删除决策模型',
		deleteConfirmMessage: '确定要删除',
		confirmHint: '按 Y 确认，按 N 或 Esc 取消',
		saveError: '保存决策模型配置失败',
	},
	customHeaders: {
		title: '自定义请求头管理',
	...
	commandPanel: {
		...
		commands: {
			...
			cleanup: '清理 ~/.snow 数据：按项目或按时间删除会话、快照、日志等',
	...
	cleanupPanel: {
		title: '清理 ~/.snow 数据',
		loading: '正在扫描磁盘占用...',
		totalUsage: '当前总占用: {size}',
		menu: {
			byProject: '按项目清理（会话、快照等）',
			byAge: '按时间清理（日志、用量、旧数据）',
		},
		menuHint: '↑↓ 选择 • Enter 进入 • ESC 关闭',
		byProjectHint:
			'↑↓ 导航 • 空格 标记/取消 • A 全选/全不选 • D 删除已标记 • ESC 返回',
		noProjects: '未找到项目数据',
		markedProjects: '已标记 {count} 个项目待删除',
		byAgeHint: '↑↓ 选择天数 • D 删除早于所选天数的数据 • ESC 返回',
		olderThan: '早于 {days} 天',
		filesCount: '{count} 个文件',
		ageIncludes: '包含: 日志、用量统计、任务、导出、会话、快照、历史、TODO 等',
		ageSelected: '已选择: 早于 {days} 天 • 约 {size} 可清理',
		confirmTitle: '确认清理',
		confirmProjects: '确定删除 {count} 个项目的全部数据吗？{names}',
		confirmAge: '确定删除早于 {days} 天的全部数据吗（约 {size}）？',
		confirmHint: 'Y 确认, N 取消',
		deleting: '正在清理中...',
		resultTitle: '清理完成',
		resultSummary: '已删除 {files} 个文件，释放 {size}',
		resultErrors: '发生 {count} 个错误:',
		resultHint: '按任意键返回菜单',
		categories: {
			sessions: '会话',
			snapshots: '快照',
